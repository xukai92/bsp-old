...
G_BSP_00 = @grammar begin
    ...
    Scalar = BasicScalar | bsub(BasicScalar, BasicScalar) | bsub(BasicScalar, BasicScalar) | bmul(BasicScalar, BasicScalar) | bdiv(BasicScalar, BasicScalar) | bpow2(BasicScalar) | bnorm(BasicVector) | bsub(Scalar, lj)
    BasicVector = pi | pj | c | vi | vj
    Vector = BasicVector | bsub(BasicVector, BasicVector) | bproject(BasicVector, BasicVector) | bdiv(BasicVector, BasicScalar) | bnormalize(BasicVector)
    Coeff = Scalar | bmul(Scalar, Scalar) | bdiv(Scalar, Scalar)
    # Force; defined as magnitude * [condition *] direction
    Bool = does_collide(si, pi, sj, pj) | is_on(si, pi, sj, pj) # condition
    BasicForce = Const * bmul(Coeff, Vector)                    # uncondiitonal force
    BasicForce = Const * bmul(bmul(Coeff, Vector), Bool)        #   conditional force
    Force = BasicForce #| badd(Force, BasicForce)
end
...
G_BSP_01 = @grammar begin
    ...
    BasicScalar = ui | uj | mi | mj
    Scalar = BasicScalar | bsub(BasicScalar, BasicScalar) | bsub(BasicScalar, BasicScalar) | bmul(BasicScalar, BasicScalar) | bdiv(BasicScalar, BasicScalar) | bpow2(BasicScalar) | bnorm(BasicVector) | bsub(Scalar, lj)
    BasicVector = bsub(pi, pj) | bsub(pi, c) | bsub(pj, c) | vi | vj | bsub(vi, vj)
    Vector = BasicVector | bproject(BasicVector, BasicVector) | bdiv(BasicVector, BasicScalar) | bnormalize(BasicVector)
    Coeff = Scalar | bmul(Scalar, Scalar) | bdiv(Scalar, Scalar)
    # Force; defined as magnitude * [condition *] direction
    Bool = does_collide(si, pi, sj, pj) | is_on(si, pi, sj, pj) # condition
    BasicForce = Const * bmul(Coeff, Vector)                    # uncondiitonal force
...
G_BSP_10 = @grammar begin
    ...
    MeterSec = bnorm(MeterSecVec) | bproject(MeterSecVec, UnitlessVec)
    MeterSecSq = bpow2(MeterSec)
    TransInvVec = MeterVec | MeterSecVec | bsub(MeterVec, MeterVec) | bsub(MeterSecVec, MeterSecVec)
    UnitlessVec = bnormalize(TransInvVec) | bdiv(MeterVec, Meter) | bdiv(MeterSecVec, MeterSec)
    BasicCoeff = Unitless | Kg | KgSq | Meter | MeterSq | MeterSec | MeterSecSq | bsub(Meter, lj)
