...
@cast function em(dataset::String)
    Threads.@threads for (seed, shuffleseed) in cproduct(1:5, 1:5)
        `julia scripts/runexp.jl em $dataset 5 3 --seed=$seed --shuffleseed=$shuffleseed --slient --logging` |> try_run
    end
end

@cast function ablation(dataset::String)
    Threads.@threads for (ntrains, seed, shuffleseed, priordim, priortrans) in cproduct(1:10, 1:5, 1:5, [false, true], [false, true])
...
@cast function ullman(wids::Int, wide::Int)
    cmds = [
        `julia scripts/runexp_ullman.jl $wid $sid 3` for wid in wids:wide, sid in 1:6
    ]
    Threads.@threads for cmd in cmds
        try_run(cmd)
    end
